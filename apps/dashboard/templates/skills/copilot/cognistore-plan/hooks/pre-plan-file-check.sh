...
case "$FILENAME" in
  plan.md|plan.txt|plans.md|implementation-plan.md|implementation_plan.md|todo-plan.md|task-plan.md|roadmap.md|*.plan.md)
    cat <<'EOF'
{
  "systemMessage": "[CogniStore] WARNING: You are writing to a file that looks like a plan. Plans MUST be created via mcp__cognistore__createPlan() — NEVER as local files. Use createPlan() with a tasks array instead. Local plan files will not be tracked, searchable, or visible in the dashboard."
}
EOF
    exit 0
    ;;
