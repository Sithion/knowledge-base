#!/usr/bin/env bash
# Stop hook: Fires at session end. Context-aware based on capture marker.
...
if [ -f "$CAPTURED_MARKER" ]; then
  # Knowledge was captured — light reminder about patterns only
  cat <<'EOF'
{
